...
communitiesLinkResultData = {
	"Base Model": ["GCN"] * 4 + ["GraphSage"] * 4 + ["GAT"] * 4 + ["GIN"] * 4 + ["P-GNN-F-2L"] * 4 + ["P-GNN-E-2L"] * 4,
	"Position Encoding": ["None", "Hash", "MSE", "Both"] * 6,
...
emailLinkResultData = {
	"Base Model": ["GCN"] * 4 + ["GraphSage"] * 4 + ["GAT"] * 4 + ["GIN"] * 4 + ["P-GNN-F-2L"] * 4 + ["P-GNN-E-2L"] * 4,
	"Position Encoding": ["None", "Hash", "MSE", "Both"] * 6,
	"AUC": [0.709, 0.767, 0.721, 0.782, 0.571, 0.735, 0.557, 0.769, 0.538, 0.758, 0.550, 0.769, 0.724, 0.785, 0.791, 0.808, 0.751, 0.769, 0.835, 1.0, 0.792, 0.770, 0.765, 0.775],
	"Kendall's Tau": [0.239, 0.364, 0.217, 0.364, 0.147, 0.361, 0.156, 0.363, 0.086, 0.336, 0.096, 0.353, 0.402, 0.443, 0.429, 0.452, 0.503, 0.529, 0.516, 1.0, 0.547, 0.549, 0.539, 0.550]
}
...
emailLinkPairResultData = {
	"Base Model": ["GCN"] * 4 + ["GraphSage"] * 4 + ["GAT"] * 4 + ["GIN"] * 4 + ["P-GNN-F-2L"] * 4 + ["P-GNN-E-2L"] * 4,
	"Position Encoding": ["None", "Hash", "MSE", "Both"] * 6,
	"AUC": [0.518, 0.681, 0.575, 0.708, 0.538, 0.693, 0.533, 0.744, 0.507, 0.725, 0.528, 0.747, 0.723, 0.741, 0.726, 0.774, 0.751, 0.734, 0.772, 1.0, 0.735, 0.784, 0.772, 0.753],
	"Kendall's Tau": [0.238, 0.432, 0.267, 0.437, 0.139, 0.428, 0.206, 0.433, 0.083, 0.407, 0.112, 0.435, 0.479, 0.525, 0.482, 0.521, 0.576, 0.615, 0.609, 1.0, 0.603, 0.638, 0.612, 0.614]
}

resultDataAll = {
	"Dataset": 24 * ["Communities"] + 24 * ["Email"] + 24 * ["Communities"] + 24 * ["Email"],
	"Task": 24 * 2 * ["Link Prediction"] + 24 * 2 * ["Pairwise Node Classification"],
	"Base Model": 4 * communitiesLinkResultData["Base Model"],
	"Position Encoding": 4 * communitiesLinkResultData["Position Encoding"],
...
base_models = ["GCN", "GraphSage", "GAT", "GIN", "P-GNN-F-2L", "P-GNN-E-2L"]
...
dfLP = DF.loc[(DF["Task"] == "Link Prediction") & ((DF["Position Encoding"] == "None") | (DF["Position Encoding"] == "Both"))]
dfNC = DF.loc[(DF["Task"] == "Pairwise Node Classification") & ((DF["Position Encoding"] == "None") | (DF["Position Encoding"] == "Both"))]
...
for l, df in {"LP": dfLP, "NC": dfNC}.items():
	...
	# sns.lineplot(x="Kendall's Tau", y="AUC", data=dfLP.loc[(DF["Dataset"] == "Communities") &  (DF["Base Model"] == "GraphSage")], ax=ax[0], legend=False)
	...
	# sns.lineplot(x="Kendall's Tau", y="AUC", data=dfLP.loc[(DF["Dataset"] == "Email") &  (DF["Base Model"] == "GraphSage")], ax=ax[1], legend=False)
