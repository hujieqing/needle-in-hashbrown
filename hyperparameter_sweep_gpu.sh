...
EPOCH_NUM=50
REPEAT_NUM=5
TMP_DIR_NAME="sweep_results/tmp_${MODEL}_${DATASET}"
...
do
  for WEIGHT_DECAY in 0.01 0.001 0.0001;
    do
	PREFIX="${LAYER_NUM}_${DATASET}_L1${LAMBDA1}_L2${LAMBDA2}_A${ALPHA}_LR${LR}_WD${WEIGHT_DECAY}"
	LOG_FILE_NAME=`echo $PREFIX | sed 's/\./d/g'`
	LOG_FILE_PATH="${TMP_DIR_NAME}/${LOG_FILE_NAME}"
	SUMMARY_PATH="${TMP_DIR_NAME}/${LOG_FILE_NAME}_summary"
