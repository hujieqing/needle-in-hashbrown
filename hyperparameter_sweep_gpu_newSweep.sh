...
EPOCH_NUM=2000
REPEAT_NUM=2
TMP_DIR_NAME="sweep_results/tmp_${MODEL}E_${DATASET}_combined"
...
for LAMBDA1 in 0 0.1 1 10;
  ...
  for LAMBDA2 in 0 0.1 1 10;
	...
	SUMMARY_PATH="${TMP_DIR_NAME}/summary"
        python main.py --model $MODEL --cpu --hash_concat --layer_num $LAYER_NUM --dataset $DATASET --lambda1 $LAMBDA1 --lambda2 $LAMBDA2 --alpha $ALPHA --lr $LR --epoch_num $EPOCH_NUM --epoch_log $EPOCH_LOG --repeat_num $REPEAT_NUM --weight_decay $WEIGHT_DECAY --early_stopping True |tee -a $LOG_FILE_PATH
